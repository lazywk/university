...
export const fetchDrivers = createAsyncThunk('settings/fetchDrivers', async () => {
    return (await http.get('operator/drivers/')).data
})

export const fetchDriverDetail = createAsyncThunk('settings/fetchDriverDetail', async (id: number) => {
    return (await http.get(`operator/drivers/${id}/`)).data
})

export const createDriver = createAsyncThunk('settings/fetchDrivers', async (data: any, { rejectWithValue }) => {
    try {
        const response = await http.post(`operator/drivers/`, data);
